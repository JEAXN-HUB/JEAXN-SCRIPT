...
-- FUNCIONES FLY Y SPEED (ARREGLADO)
local flying = false
local speedOn = false
...
local function toggleFly()
    ...
    if flying then
        ...
        flyConn = RunService.Heartbeat:Connect(function()
            ...
            -- PC
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then vel = vel + cam.LookVector * 35 end
            if UserInputService:IsKeyDown(Enum.KeyCode.S) then vel = vel - cam.LookVector * 35 end
            if UserInputService:IsKeyDown(Enum.KeyCode.A) then vel = vel - cam.RightVector * 35 end
            if UserInputService:IsKeyDown(Enum.KeyCode.D) then vel = vel + cam.RightVector * 35 end
            -- MOBILE JOYSTICK FIX
            local moveDir = hum.MoveDirection
            if moveDir.Magnitude > 0.1 then
                vel = vel + (cam.RightVector * moveDir.X + cam.LookVector * -moveDir.Z) * 35
            end
            if UserInputService:IsKeyDown(Enum.KeyCode.Space) or _G.JEAXN_UP then
                vel = vel + Vector3.new(0,35,0)
            end
            if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or _G.JEAXN_DOWN then
                vel = vel - Vector3.new(0,35,0)
            end
            flyBV.Velocity = vel
            flyBG.CFrame = cam
...
local function loadJeaxnHub()
    ...
    upBtn.Position = UDim2.new(0.05,0,0,65)
    upBtn.Text = "SUBIR"
    ...
    downBtn.Size = UDim2.new(0.42,0,0,28)
    downBtn.Position = UDim2.new(0.53,0,0,65)
    downBtn.Text = "BAJAR"
    ...
    speedBtn.Position = UDim2.new(0.05,0,0,100)
    ...
    upBtn.MouseButton1Down:Connect(function() _G.JEAXN_UP = true end)
    upBtn.MouseButton1Up:Connect(function() _G.JEAXN_UP = false end)
    downBtn.MouseButton1Down:Connect(function() _G.JEAXN_DOWN = true end)
    downBtn.MouseButton1Up:Connect(function() _G.JEAXN_DOWN = false end)
    ...
    -- SOLO CAMBIAR EL TITULO QUE MARCASTE
    task.spawn(function()
        while true do
            pcall(function()
                for _, obj in pairs(game:GetService("CoreGui"):GetDescendants()) do
                    if obj:IsA("TextLabel") and obj.Text == "Snipe or Die - Syrex Hub" then
                        obj.Text = "JEAXN HUB"
                    end
                end
            end)
